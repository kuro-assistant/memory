...
class DecayEngine:
    ...
    def apply_decay(self):
        ...
        with self.db.conn:
            # Fetch all atoms
            cursor = self.db.conn.execute("SELECT id, magnitude, last_updated, decay_rate FROM memory_atoms")
            ...
            for atom_id, magnitude, last_updated_str, decay_rate in atoms:
                ...
                # If magnitude is negligible, we could prune it
                if abs(new_magnitude) < 0.01:
                    self.db.conn.execute("DELETE FROM memory_atoms WHERE id = ?", (atom_id,))
                else:
...
class ReinforcementEngine:
    ...
    def reinforce(self, key: str, choice: bool, magnitude=0.1):
        """
        choice: True for approval (+), False for rejection (-)
        """
